...
{
    MB_LEFT     = 0x01, /* left-center */
    MB_TOP      = 0x02, /* top-center */
    MB_TOPRIGHT = 0x04, /* top-right */
    MB_TOPLEFT  = 0x08, /* top-left */

    MB_PRIVATE  = 0x10, /* reserved */

    ALL_NEIGHBORS = 0xf, /* LEFT|TOP|TOPRIGHT|TOPLEFT */
};
...
enum mb_class_e
{
    ...
    B_DIRECT        = 7,
    B_L0_L0         = 8,  /* for B16x16, B16x8, B8x16 */
    B_L0_L1         = 9,  /* for B16x8, B8x16 */
    B_L0_BI         = 10, /* for B16x8, B8x16 */
    B_L1_L0         = 11, /* for B16x8, B8x16 */
    B_L1_L1         = 12, /* for B16x16, B16x8, B8x16 */
    B_L1_BI         = 13, /* for B16x8, B8x16 */
    B_BI_L0         = 14, /* for B16x8, B8x16 */
    B_BI_L1         = 15, /* for B16x8, B8x16 */
    B_BI_BI         = 16, /* for B16x16, B16x8, B8x16 */
    B_8x8           = 17, /* for B8x8 */
    B_SKIP          = 18,
...
/* pixel position of 4x4 block in encode frame buffer */
...
/* pixel position of 4x4 block in decode frame buffer */
...
/**************************************************************************************
 * qp mapping table of luma => chroma                                                 *
 * x264 only encodes luma and chroma planes at the same quantizer up to quantizer 29. *
 * After this, chroma is progressively quantized by a lower amount than luma until    *
 * end with luma at 51 and chroma at 39. This behavior is required by H.264 standard. *
 **************************************************************************************/
...
static ALWAYS_INLINE int x264_mb_predict_intra4x4_mode( x264_t *h, int idx )
{
	 ...
	 * 1. get the minimum predict mode value of left mb and top mb
	 * 2. if left mb or top mb doesn't exist, use dc instead.
	 */
    const int ma = h->mb.cache.intra4x4_pred_mode[x264_scan8[idx] - 1]; /* predict mode of left neighbor */
    const int mb = h->mb.cache.intra4x4_pred_mode[x264_scan8[idx] - 8]; /* predict mode of top neighbor */
    const int m  = X264_MIN( x264_mb_pred_mode4x4_fix(ma), x264_mb_pred_mode4x4_fix(mb) );
...
{
    const int za = h->mb.cache.non_zero_count[x264_scan8[idx] - 1]; /* non-zero count of left neighbor */
    const int zb = h->mb.cache.non_zero_count[x264_scan8[idx] - 8]; /* non-zero count of top neighbor */
...
{
	/* check if user enables 8x8 transform */
    if( !h->pps->b_transform_8x8_mode )
        return 0;

    /* check if current mb type supports 8x8 transform */
    if( h->mb.i_type != P_8x8 )
        return x264_transform_allowed[h->mb.i_type];

    /* check if 4 sub-blocks of P_8x8 are all D_L0_8x8 */
