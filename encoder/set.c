...
void x264_sps_init( x264_sps_t *sps, int i_id, x264_param_t *param )
{
    ...
    int max_frame_num;
	sps->i_id = i_id;
    sps->i_mb_width = ( param->i_width + 15 ) >> 4;
    sps->i_mb_height= ( param->i_height + 15 ) >> 4;
    sps->i_chroma_format_idc = csp >= X264_CSP_I444 ? CHROMA_444 :
                               csp >= X264_CSP_I422 ? CHROMA_422 : CHROMA_420;
    sps->b_qpprime_y_zero_transform_bypass = param->rc.i_rc_method == X264_RC_CQP && param->rc.i_qp_constant == 0;

    /**************************************************************************
     * H.264 profile indication decide flow:                                  *
     *                                                                        *
     * 1. Set High444 if in lossless mode or CHROMA_444                       *
     * 2. Set High422 if CHROMA_422                                           *
     * 3. Set High10 if HIGH_BIT_DEPTH                                        *
     * 4. Set High if 8x8 transform enabled or CQM is NOT Flat                *
     * 5. Set Main if CABAC or B-frames or Interlace or Weighted-pred enabled *
     * 6. Otherwise, use Baseline as fallback.                                *
     **************************************************************************/
    ...
    sps->vui.b_nal_hrd_parameters_present = !!param->i_nal_hrd; // same as param->i_nal_hrd
    sps->vui.b_pic_struct_present = param->b_pic_struct; // same as param->b_pic_struct
...
void x264_pps_write( bs_t *s, x264_sps_t *sps, x264_pps_t *pps )
{
    ...
    /* following features are used only in high profile */
...
int x264_sei_version_write( x264_t *h, bs_t *s )
{
    ...
    sprintf( payload+16, "h264 - core %d - H.264/MPEG-4 AVC codec - options: %s", X264_BUILD, opts );
...
{
	/*******************************************************************
	 * level_idc:   level indication                                   *
	 * mbps:        max macroblock processing rate (macroblocks/sec)   *
	 * frame_size:  max frame size (macroblocks)                       *
	 * dpb:         max decoded picture buffer (bytes)                 *
	 * bitrate:     max bitrate (kbit/sec)                             *
	 * cpb:         max vbv buffer (kbit)                              *
	 * mv_range:    max vertical mv component range (pixels)           *
	 * mvs_per_2mb: max mvs per 2 consecutive mbs.                     *
	 * slice_rate:                                                     *
	 * mincr:       min compression ratio                              *
	 * bipred8x8:   limit bipred to >=8x8                              *
	 * direct8x8:   limit b_direct to >=8x8                            *
	 * frame_only:  forbid interlacing                                 *
	 *******************************************************************/
...
{
    int ret = 0;
    int mbs = h->sps->i_mb_width * h->sps->i_mb_height;          /* macro-blocks per frame */
    int dpb = mbs * 384 * h->sps->vui.i_max_dec_frame_buffering; /* decode picture buffer */
